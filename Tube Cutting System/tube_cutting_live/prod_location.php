...
<head>
	...
	<title>Production Location (Tube Cutting)</title>
	<!-- Title Logo -->
	<!-- Font Awesome -->
...
<?php
	...
	include 'Modal/search_admin_kbn_prod_loc.php';
	include 'Modal/master_delete_confirm.php';
?>
<div class="row ml-0 mr-0 card_opa">
	...
	<div class="col-sm-4 col-md-4 col-lg-4 text-center mt-1">
		<label class="h3"><i class="fas fa-th"></i> Parts Location</label>	
	</div>
	<div class="col-sm-12 col-md-12 col-lg-12">
		<button class="btn btn-info btn-file float-right">
			<form method="post" action="Import/excelUploadProdLoc.php" enctype="multipart/form-data">
		...
		<a href="Format/Production Location Format.xlsx" download><button class="btn btn-info float-right"><i class="fas fa-arrow-down"></i> Download Format</button></a>
	...
	<div class="col-sm-12 col-md-12 col-lg-12 text-center mt-1" id="pending_parts_label_section">
		<label class="h4"><i class="fas fa-th"></i> Kanban with Parts Location</label>	
	</div>
	<div class="row mx-0 col-sm-12 col-md-12 col-lg-12" id="pending_parts_section">
		...
		<div class="col-sm-12 col-md-12 col-lg-12 text-center">
			<div class="loader_popup text-info text-center" id="loading_master" style="display:none;">Loading....</div>
		</div>
		<div class="my-0 mx-0 col-sm-12 col-md-12 col-lg-12">
			<label class="col-sm-12 col-md-12 col-lg-12 text-right" id="counter_viewer_master"></label>
		</div>
		<input type="hidden" id="loader_count_master">
		<div class="rounded-circle info-color card-img-100 mx-auto mb-1 pulse" id="see_more_master" style="margin-top:-10px;width:50px;height:50px;cursor:pointer;display:none;" onclick="see_more_master()" data-toggle="tooltip" data-placement="right" title="See More">
...
function upload_csv(){
	...
	$.ajax({
		url: 'Import/excelUploadProdLoc.php', // point to server-side PHP script 
		...
		success: function (response){
			if(response == 'Updating Sucessfully'){
...
<script>
	...
	function line_no_search_action(){
		...
		xhttp.open("GET", "AJAX/search_parts_prod_loc.php?operation=display_all&&line_no_search="+line_no_search+"&&parts_code_search1="+parts_code_search1+"&&parts_name_search1="+parts_name_search1+"&&length_search1="+length_search1, true);
		xhttp.send();
	}
	...
	function load_all_station(){
		...
		xhttp.open("GET", "AJAX/search_parts_prod_loc.php?operation=load_all_station", true);
		xhttp.send();
	}
...
<script>
	...
	// 	}
	// 	};
	// 	xhttp.open("GET", "AJAX/kanban_masterlist_location.php?operation=open_details&&id="+x, true);
	// 	xhttp.send();
	// }
	...
	// 	}
	// 	};
	// 	xhttp.open("GET", "AJAX/kanban_masterlist_location.php?operation=delete_kanban&&&&id_hidden="+id_hidden, true);
	// 	xhttp.send();
	// }
	function select_all(){
		var xhttp = new XMLHttpRequest();
		...
		if(this.readyState == 4 && this.status == 200){
			...
			document.getElementById('loader_count_master').value=100;
			count_all_master();
		}
		};
		xhttp.open("GET", "AJAX/kanban_masterlist_location.php?operation=select_all&&limiter_master=0", true);
		xhttp.send();
	}
	function count_all_master(){
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			var response = this.responseText;
			var total_count = parseInt(response);
			let table_rows = parseInt(document.getElementById("kanban_list").rows.length);
			let loader_count_master = document.getElementById('loader_count_master').value;
			document.getElementById('counter_viewer_master').innerHTML=table_rows +' - '+ total_count;
			if (total_count == 0){
				document.getElementById('see_more_master').style.display='none';
			}else if (total_count > loader_count_master){
				document.getElementById('see_more_master').style.display='inline-block';
			}else if (total_count < loader_count_master){
				document.getElementById('see_more_master').style.display='none';
			}
		}
		};
		xhttp.open("GET", "AJAX/kanban_masterlist_location.php?operation=count_all_master", true);
		xhttp.send();
	}
	function see_more_master(){
		var loader_count_master = parseInt(document.getElementById('loader_count_master').value);
		var xhttp = new XMLHttpRequest();
		xhttp.onreadystatechange = function(){
		if(this.readyState == 4 && this.status == 200){
			var response = this.responseText;
			document.getElementById('kanban_list').innerHTML += response;
			document.getElementById('loader_count_master').value = loader_count_master + 100;
			count_all_master();
		}
		};
		xhttp.open("GET", "AJAX/kanban_masterlist_location.php?operation=select_all&&limiter_master="+loader_count_master, true);
		xhttp.send();
	}
	...
	function delete_single_kanban_master(id){
		$("#Master_Delete_Confirm").modal();
	}
	function export_all_excel_search(){
		var line_no_search = document.getElementById('line_no_search').value;
		var parts_code_search1 = document.getElementById('parts_code_search1').value;
		var parts_name_search1 = document.getElementById('parts_name_search1').value;
		var length_search1 = document.getElementById('length_search1').value;
		window.open('export_search_prod_loc.php?line_no='+line_no_search+'&&parts_code='+parts_code_search1+'&&parts_name='+parts_name_search1+'&&length='+length_search1,'_blank');
	}
	function print_single_kanban(x){
		window.open('print_kanban_missing.php?id='+x,'_blank');
	}
