	...
	<div class="col-sm-12 col-md-12 col-lg-12 mt-0">
		<div class="md-form mb-0 col-sm-3 float-right mt-0">
			<select class="browser-default custom-select form-control" id="status_sender_id" onchange="status_sender()">
				<option>Tubecutting Remarks</option>
				<option selected>Production Remarks</option>
			</select>
		</div>
	</div>
...
function display_remarks(){
	...
	xhttp.open("GET", "AJAX/notification_tc.php?operation=display_remarks&&status_sender_id="+status_sender_id, true);
	xhttp.send();
}
function save_mm_remarks1(){
    ...
    var remarks_scooter_station = document.getElementById('remarks_scooter_station1').value;
	var xhttp = new XMLHttpRequest();
	xhttp.onreadystatechange = function(){
	if(this.readyState == 4 && this.status == 200){
		var response = this.responseText;
...
function add_remarks_tc_notif_pr(x){
    ...
    var remarks_scooter_station1 = split[6];
    ...
    document.getElementById('remarks_scooter_station1').value = remarks_scooter_station1;
...
xhttp.open("GET", "AJAX/realtime_remarks.php?operation=read_remarks_now_mm&&id_scanned_kanban="+id_scanned_kanban+"&&kanban="+kanban+"&&kanban_num="+kanban_num+"&&scan_date_time="+scan_date_time+"&&request_date_time="+request_date_time, true);
	xhttp.send();
}
function close_modal_remarks1(){
    ...
    document.getElementById('remarks_real_time_status1').value='';
    document.getElementById('remarks_scooter_station1').value='';
    real_time_refresh();
}
...
function print_kanban_notif(x){
    window.open('print_kanban_notif.php?id='+x,'_blank');
}
</script>
<!-- JQuery -->
...
<script type="text/javascript" src="myjs/real_time_notif_tc.js"></script>
