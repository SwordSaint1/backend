...
$datenow = date('Y-m-d');
$filename = "History Results".$datenow.".xls";
...
$sql = "SELECT * FROM tc_history WHERE status != '' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND comment LIKE '%$comment%' AND length LIKE '%$length%' AND kanban_no LIKE '%$kanban_no%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC LIMIT $limiter";
