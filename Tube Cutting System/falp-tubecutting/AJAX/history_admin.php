...
if($operation == "Scooter Station"){
	...
	$date_to = date("Y-m-d");
	$date_to = $date_to.' 24:59:59';
	$sql = "SELECT * FROM tc_history WHERE (store_out_date_time >= '$date_from' AND store_out_date_time <= '$date_to') ORDER BY id DESC LIMIT 20";
	$result = $conn_sql->query($sql);
	if($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			$kanban =$row['kanban'];
			...
			echo'
				<tr>
					<td class="font-weight-normal">'.$no.'</td><td class="font-weight-normal">'.$row['serial_no'].'</td><td class="font-weight-normal">'.$row['stock_address'].'</td><td class="font-weight-normal">'.$row['parts_code'].'</td><td class="font-weight-normal">'.$row['parts_name'].'</td><td class="font-weight-normal">'.$row['length'].'</td><td class="font-weight-normal">'.$row['quantity'].'</td><td class="font-weight-normal">'.$row['kanban_no'].'</td><td class="font-weight-normal">'.$row['store_out_date_time'].'</td><td class="font-weight-normal">'.$new_iden_qr.'</td>
				</tr>
			';
		}
	}else{
