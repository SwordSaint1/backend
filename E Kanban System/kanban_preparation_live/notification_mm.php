...
const print_kanban_notif =(data_param)=>{
	let split =data_param.split('~!~');
	let id = split[0];
	let status = split[1];
	if (status == 'Pending'){
		window.open('print_notif_pending.php?id='+id,'_blank');
	}else{
		window.open('print_notif_op.php?id='+id,'_blank');
	}
	setTimeout(display_remarks_after_seen, 5000);
}
const add_remarks_mm =(data_param)=>{
	let split = data_param.split('~!~');
...
const update_seen =()=>{
	let status_sender_id = document.getElementById('status_sender_id').value;
	if(status_sender_id == 'Production Remarks'){
		let remarks_id_scanned_kanban = document.getElementById('remarks_id_scanned_kanban').value;
		let remarks_kanban = document.getElementById('remarks_kanban').value;
		let remarks_kanban_num = document.getElementById('remarks_kanban_num').value;
		let remarks_scan_date_time = document.getElementById('remarks_scan_date_time').value;
		...
		xhttp.send();
	}else if(status_sender_id == 'MM Remarks'){
	}else{
	}
}
...
<script type="text/javascript" src="myjs/real_time_notif_mm.js"></script>
<!-- My JavaScript for Conversation Updates-->
