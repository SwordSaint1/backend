...
$datenow = date('Y-m-d');
$filename = $search_status." - ".$datenow.".xls";
...
if($search_status == 'Pending'){
    $sql = "SELECT TOP $limiter_search id, id_scanned_kanban, kanban, line_no, stock_address, parts_code, parts_name, quantity, kanban_num, scooter_station, scan_date_time, requested_by, request_date_time, status FROM mm_scanned_kanban WHERE status = 'Pending' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC";
}elseif($search_status == 'Ongoing Picking'){
    $sql = "SELECT TOP $limiter_search id, id_scanned_kanban, kanban, line_no, stock_address, parts_code, parts_name, quantity, kanban_num, scooter_station, scan_date_time, requested_by, request_date_time, print_date_time, status FROM mm_ongoing_picking WHERE status = 'Ongoing Picking' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC";
}
