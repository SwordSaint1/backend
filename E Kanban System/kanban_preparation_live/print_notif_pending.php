...
<body class="mt-0 mb-0">
	<?php
		$page_num = 0;
		$id = $_GET['id'];
		$sql = "SELECT * FROM mm_scanned_kanban WHERE id='$id' ORDER BY line_no ASC, stock_address ASC , parts_code ASC";
		...
		if ($row === true){
			while($rows = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
				...
				$sql5 = "UPDATE mm_remarks SET remarks_status=?, seen_date_time=? WHERE id_scanned_kanban = ? AND kanban = ? AND kanban_num = ? AND scan_date_time = ? AND request_date_time = ? AND sender_identification = ?";
				$params5 = array('Read', $seen_date_time, $id_scanned_kanban, $kanban, $kanban_num, $scan_date_time, $request_date_time, 'Production Remarks');
				$stmt5 = sqlsrv_query($conn_sqlsrv, $sql5, $params5);
				$rows_affected5 = sqlsrv_rows_affected($stmt5);
				if($rows_affected5 === false){
					die(print_r( sqlsrv_errors(), true));
				}elseif( $rows_affected5 == -1){
					//echo "Seen";
				}else{
