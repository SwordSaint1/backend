...
if($operation == "display_all"){
    ...
    $sql = "SELECT id, id_scanned_kanban, kanban, line_no, parts_code, parts_name, kanban_num, quantity, scooter_station, scan_date_time, request_date_time, print_date_time, store_out_date_time FROM mm_history WHERE status != '' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND scooter_station LIKE '%$scooter_station%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC OFFSET $limiter_count ROWS FETCH NEXT 20 ROWS ONLY";
	...
	$date_to=date_create($date_to);
	$date_to= date_format($date_to,"Y-m-d H:i:s");
	$sql = "SELECT id FROM mm_history WHERE status != '' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND scooter_station LIKE '%$scooter_station%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY id ASC";
    $params = array();
