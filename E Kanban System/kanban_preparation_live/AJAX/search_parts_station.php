...
if($operation == "display_all"){
	...
	$no = $limiter_search;
	 if($search_status == 'Pending'){
		$sql = "SELECT id_scanned_kanban, kanban, line_no, stock_address, parts_code, parts_name, quantity, kanban_num, scooter_station, scan_date_time, request_date_time, status FROM mm_scanned_kanban WHERE scooter_station='$scooter_station' AND status = 'Pending' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC OFFSET $limiter_search ROWS FETCH NEXT 20 ROWS ONLY";
	 }elseif($search_status == 'Ongoing Picking'){
		$sql = "SELECT id_scanned_kanban, kanban, line_no, stock_address, parts_code, parts_name, quantity, kanban_num, scooter_station, scan_date_time, request_date_time, print_date_time, status FROM mm_ongoing_picking WHERE scooter_station='$scooter_station' AND status = 'Ongoing Picking' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC OFFSET $limiter_search ROWS FETCH NEXT 20 ROWS ONLY";
	 }
    ...
    if ($row === true){
        while($rows = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
            ...
            echo'
 				<tr>
                    <td>'.$no.'</td><td>'.$rows['line_no'].'</td><td>'.$rows['stock_address'].'</td><td>'.$rows['parts_code'].'</td><td>'.$rows['parts_name'].'</td><td>'.$rows['kanban_num'].'</td><td>'.$rows['quantity'].'</td><td>'.$rows['scooter_station'].'</td><td>'.$truck_no.'</td><td>'.date_format($rows['scan_date_time'],"Y-m-d H:i:s").'</td><td>'.date_format($rows['request_date_time'],"Y-m-d H:i:s").'</td><td>'.$print_date_time.'</td><td style="cursor:pointer;" onclick="add_remarks_distributor(&quot;'.$rows['id_scanned_kanban'].'~!~'.$rows['kanban'].'~!~'.$rows['kanban_num'].'~!~'.date_format($rows['scan_date_time'],"Y-m-d H:i:s").'~!~'.date_format($rows['request_date_time'],"Y-m-d H:i:s").'~!~'.$rows['scooter_station'].'&quot;)">'.$mm_remarks.'</td><td>'.$rows['status'].'</td>
 				</tr>
 			';
        }
    }else{
	...
	$parts_name_search1 = $_GET['parts_name_search1'];
	if($search_status == 'Pending'){
		$sql = "SELECT id FROM mm_scanned_kanban WHERE scooter_station='$scooter_station' AND status = 'Pending' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY id ASC";
	}elseif($search_status == 'Ongoing Picking'){
		$sql = "SELECT id FROM mm_ongoing_picking WHERE scooter_station='$scooter_station' AND status = 'Ongoing Picking' AND line_no LIKE '%$line_no_search%' AND parts_name LIKE '%$parts_name_search1%' AND parts_code LIKE '%$parts_code_search1%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY id ASC";
	 }
    $params = array();
