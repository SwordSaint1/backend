...
if($operation == "count_entries"){
	...
	$sql = "SELECT id FROM mm_scanned_kanban WHERE scooter_station='$scooter_station' AND status = 'Pending'";
    $params = array();
...
}else if($operation == "count_op_realtime"){
    $scooter_station = $_GET['scooter_station'];
	$sql = "SELECT id FROM mm_ongoing_picking WHERE scooter_station='$scooter_station' AND status = 'Ongoing Picking'";
    $params = array();
	$options =  array( "Scrollable" => SQLSRV_CURSOR_KEYSET );
	$stmt = sqlsrv_query($conn_sqlsrv,$sql,$params,$options );
	$row_count = sqlsrv_num_rows($stmt);
    ...
    sqlsrv_free_stmt($stmt);
    sqlsrv_close($conn_sqlsrv);
}else if($operation == "count_all_notification"){
	$scooter_station = $_GET['scooter_station'];
