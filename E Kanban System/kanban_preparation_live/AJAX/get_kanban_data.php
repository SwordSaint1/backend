...
if($operation == 'get_scan_kanban'){
    ...
    $default = substr($kanban_scan, 0,5); //Default
    $location = substr($kanban_scan, 5,4); //Location
    $delivery = substr($kanban_scan, 9,4); //Delivery
    $parts_code = substr($kanban_scan, 13,20); //Parts Code
    $parts_code = trim($parts_code); //Parts Code
    $kanban_num = substr($kanban_scan, 33,4); //Kanban No
    $kanban_num = ltrim($kanban_num, '0'); //Kanban No
    $line_no = substr($kanban_scan, 37,10); //Line No
    $line_no = trim($line_no); //Line No
    $kanban_num_test = is_numeric($kanban_num);
    if($kanban_num_test == 1){
        // Check Kanban on FSIB
        $query = oci_parse($conn_oracle, "SELECT C_BHNSZICOD, C_BHNSZINAM, C_DSNSIZ, C_DSNJIIRO, C_DSNSTRIPE,C_TKSSENCOD,L_MINLOTSUU,C_KNYSAKCOD,C_KNYSAKBHNNAM,NVL(C_BANTICODE,'No Data') AS C_BANTICODE,C_KNYSAKBHNNAM 
        ...
        while($row=oci_fetch_assoc($query)){
            ...
            $wire_size = $row['C_DSNSIZ'];
            $ground_color = $row['C_DSNJIIRO'];
            $stripe_color = $row['C_DSNSTRIPE'];
            $quantity = $row['L_MINLOTSUU'];
            $stock_address = $row['C_BANTICODE'];
            $supplier_name = $row['C_KNYSAKBHNNAM'];
            if($stripe_color != ""){
                $stripe_color =' / '.$stripe_color;
            }else{
                $stripe_color = $stripe_color;
            }
            $parts_name = $parts_name.' '.$wire_size.' '.$ground_color.' '.$stripe_color;
            //For Checking of Duplicate Entry
            $sql = "SELECT id FROM mm_scanned_kanban WHERE id_scanned_kanban='$id_scanned_kanban' AND kanban='$kanban_scan' AND line_no='$line_no' AND parts_code='$parts_code' AND kanban_num='$kanban_num'";
            ...
            if ($row === true){
                ...
                //For Checking of Already Requested
                $sql1 = "SELECT id FROM mm_scanned_kanban WHERE kanban='$kanban_scan' AND line_no='$line_no' AND parts_code='$parts_code' AND kanban_num='$kanban_num' AND status !='' UNION SELECT id FROM mm_ongoing_picking WHERE kanban='$kanban_scan' AND line_no='$line_no' AND parts_code='$parts_code' AND kanban_num='$kanban_num' AND status !=''";
                $stmt1 = sqlsrv_query($conn_sqlsrv, $sql1);
                $row1 = sqlsrv_has_rows($stmt1);
                if ($row1 === true){
                    $identifier = 'Error: Already Requested!';
                    echo'Error: Already Requested!';
                }else{
                    //For Inserting Request
                    $scan_date_time = date("Y-m-d H:i:s");
                    $sql2 = "INSERT INTO mm_scanned_kanban (id_scanned_kanban, kanban, location, delivery, line_no, stock_address, supplier_name, parts_code, parts_name, quantity, kanban_num, scooter_station, scan_date_time) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";
                    $params2 = array($id_scanned_kanban, $kanban_scan, $location, $delivery, $line_no, $stock_address, $supplier_name, $parts_code, $parts_name, $quantity, $kanban_num, $scooter_area, $scan_date_time);
