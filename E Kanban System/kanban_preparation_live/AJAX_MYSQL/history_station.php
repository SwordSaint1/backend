...
if($operation == "display_all"){
	...
	//Starting of Query
	$sql = "SELECT * FROM tc_history WHERE line_no LIKE '%$line_no%' AND parts_code LIKE '%$parts_code%' AND parts_name LIKE '%$parts_name%' AND comment LIKE '%$comment%' AND length LIKE '%$length%' AND scooter_station LIKE '%$scooter_station%' AND (scan_date_time >= '$date_from' AND scan_date_time <= '$date_to') ORDER BY id ASC LIMIT $load_more_counter, 20";
	...
	$scooter_station = mysqli_real_escape_string($conn_sql, $_GET['scooter_station']);
	//Starting of Query
	$sql = "SELECT count(id) as total FROM tc_history WHERE line_no LIKE '%$line_no%' AND parts_code LIKE '%$parts_code%' AND parts_name LIKE '%$parts_name%' AND comment LIKE '%$comment%' AND length LIKE '%$length%' AND scooter_station LIKE '%$scooter_station%' AND (scan_date_time >= '$date_from' AND scan_date_time <= '$date_to') ORDER BY id ASC";
	$result = $conn_sql->query($sql);
	if($result->num_rows > 0){
		while($row = $result->fetch_assoc()){
			echo $row['total'];
