...
';
 $sql = "SELECT TOP $load_more_counter id, id_scanned_kanban, kanban, line_no, stock_address, parts_code, parts_name, quantity, kanban_num, scooter_station, requested_by, scan_date_time, request_date_time, print_date_time, store_out_date_time, status FROM mm_history WHERE scooter_station LIKE '%$scooter_area_select' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to' ORDER BY scan_date_time DESC";
