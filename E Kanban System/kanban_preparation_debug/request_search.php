...
<head>
	...
	<title>E-Kanban Request Search</title>
	<!-- Title Logo -->
	<!-- Font Awesome -->
...
<div class="mx-0">
	<div class="row ml-0 mr-0">
		...
		<div class="col-sm-4 col-md-4 col-lg-4 text-center mt-1">
			<label class="h3"><i class="fas fa-search"></i> Request Search </label>	
		</div>
	</div>
	<div class="row mx-0">
		...
		<div class="md-form mb-0 col-sm-4 col-md-4 col-lg-4" id="scooter_station_select_section">
			<select id="status_select" class="browser-default form-control" onchange="status_change()">
		...
		<div class="col-sm-12 col-md-12 col-lg-12 mt-1" id='tracking_storeout_search'>
			...
			<div class="track mt-3" id='tracked_op' style='display:none;'>
				<div class="step active"> <span class="icon"> <i class="fa fa-check"></i> </span> <span class="text">Order confirmed</span> </div>
	...
	<div class="row mx-0 col-sm-12 col-md-12 col-lg-12">
		...
		<div class="rounded-circle unique-color white-text waves-effect card-img-100 mx-auto mb-1 pulse" id="load_more_botton" style="margin-top:-10px;width:50px;height:50px;cursor:pointer;display:none;" onclick="load_more_request()" data-toggle="tooltip" data-placement="top" title="Load More">
...
const search_action =()=>{
	...
	document.getElementById('tracked_op').style.display="none";
	let scooter_area_select = document.getElementById('scooter_area_select').value;
	let date_from = document.getElementById('date_from').value;
	let date_to = document.getElementById('date_to').value;
	let status_select = document.getElementById('status_select').value;
	...
	if (status_select == '' || status_select == 'Status'){
		...
		if(this.readyState == 4 && this.status == 200){
			...
			document.getElementById('load_more_botton').style.display="none";
			document.getElementById('request_section').innerHTML=response;
			document.getElementById('loader_indicator').style.display="none";
			if(status_select == 'Pending'){
				document.getElementById('tracked_pending').style.display="flex";
				document.getElementById('tracked_op').style.display="none";
			}else if(status_select == 'Ongoing Picking'){
				document.getElementById('tracked_pending').style.display="none";
				document.getElementById('tracked_op').style.display="flex";
			}
			count_all();
		}
		};
		xhttp.open("GET", "AJAX/request_search.php?operation=display_all&&scooter_area_select="+scooter_area_select+"&&date_from="+date_from+"&&date_to="+date_to+"&&status_select="+status_select+"&&line_no="+line_no+"&&parts_code="+parts_code+"&&parts_name="+parts_name+"&&limiter_count=0&&no=0", true);
		xhttp.send();
	}
}
const count_all =()=>{
	...
	if (status_select == '' || status_select == 'Status'){
		...
		xhttp.onreadystatechange = function(){
			if (this.readyState == 4 && this.status == 200){
				...
				let table_rows = document.getElementById("request_section").rows.length;
		...
		xhttp.open("GET", "AJAX/request_search.php?operation=count_all&&scooter_area_select="+scooter_area_select+"&&date_from="+date_from+"&&date_to="+date_to+"&&status_select="+status_select+"&&line_no="+line_no+"&&parts_code="+parts_code+"&&parts_name="+parts_name+"&&limiter_count="+load_more_counter, true);
		xhttp.send();
	}
}
const load_more_request =()=>{
	document.getElementById('load_more_botton').style.display="none";
	document.getElementById('loader_indicator').style.display="inline-block";
	let scooter_area_select = document.getElementById('scooter_area_select').value;
	let date_from = document.getElementById('date_from').value;
	...
	if (status_select == '' || status_select == 'Status'){
		...
		xhttp.open("GET", "AJAX/request_search.php?operation=display_all&&scooter_area_select="+scooter_area_select+"&&date_from="+date_from+"&&date_to="+date_to+"&&status_select="+status_select+"&&line_no="+line_no+"&&parts_code="+parts_code+"&&parts_name="+parts_name+"&&limiter_count="+load_more_counter, true);
		xhttp.send();
	}
}
const export_excel =()=>{
	...
	let date_from = document.getElementById('date_from').value;
	let date_to = document.getElementById('date_to').value;
	let search_status = document.getElementById('status_select').value;
	let line_no_search = document.getElementById('line_no').value;
	let parts_code_search1 = document.getElementById('parts_code').value;
	let parts_name_search1 = document.getElementById('parts_name').value;
	window.open('export_request_all_search.php?date_from='+date_from+'&&date_to='+date_to+'&&search_status='+search_status+'&&line_no_search='+line_no_search+'&&parts_code_search1='+parts_code_search1+'&&parts_name_search1='+parts_name_search1+'&&scooter_station='+scooter_station,'_blank');
}
const export_excel_showed =()=>{
	...
	let parts_code_search1 = document.getElementById('parts_code').value;
	let parts_name_search1 = document.getElementById('parts_name').value;
	let limiter_search = document.getElementById('load_more_counter').value;
	window.open('export_request_result_search.php?date_from='+date_from+'&&date_to='+date_to+'&&search_status='+search_status+'&&line_no_search='+line_no_search+'&&parts_code_search1='+parts_code_search1+'&&parts_name_search1='+parts_name_search1+'&&limiter_search='+limiter_search+'&&scooter_station='+scooter_station,'_blank');
}
get_all_station();
</script>
</html>