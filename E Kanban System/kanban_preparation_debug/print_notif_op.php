...
<body class="mt-0 mb-0">
	<?php
		$page_num = 0;
		$id = $_GET['id'];
		$sql = "SELECT * FROM mm_ongoing_picking WHERE id='$id' ORDER BY line_no ASC, stock_address ASC , parts_code ASC";
			...
			while($rows = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
				...
				echo'
					...
					<tr>
						...
						<td rowspan="2" class="p-0 pl-2 pt-0 pb-0"><span class="font-weight-bold" style="font-size: 18px;"> Kanban No.: </span><span style="font-size: 40px;" class="font-weight-bold pl-2">'.$kanban_num.'</span><br><span style="font-size: 18px;" class="font-weight-bold pull-bottom pr-2 float-right">'.$page_num.' R</span></td>
					</tr>
					<tr>
				...
				$sql3 = "UPDATE mm_remarks SET remarks_status=?, seen_date_time=? WHERE id_scanned_kanban = ? AND kanban = ? AND kanban_num = ? AND scan_date_time = ? AND request_date_time = ? AND sender_identification = ?";
				$params3 = array('Read', $seen_date_time, $id_scanned_kanban, $kanban, $kanban_num, $scan_date_time, $request_date_time, 'Production Remarks');
				...
				$reprint_date_time = date("Y-m-d H:i:s");
				$sql3 = "INSERT INTO mm_reprint_kanban (id_scanned_kanban, kanban, scooter_station, scan_date_time, request_date_time, reprint_date_time, status) VALUES (?,?,?,?,?,?,?)";
				$params3 = array($id_scanned_kanban, $kanban, $scooter_station, $scan_date_time, $request_date_time, $reprint_date_time, 'Ongoing Picking');
				$stmt3 = sqlsrv_query($conn_sqlsrv, $sql3, $params3);
				if($stmt3 === false){
					die( print_r( sqlsrv_errors(), true));
				}else{
