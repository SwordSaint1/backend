...
if($operation == "display_all"){
    ...
    if ($row === true){
        while($rows = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC)){
            ...
            echo'
 				<tr>
                    <td>'.$no.'</td><td>'.$rows['line_no'].'</td><td>'.$rows['stock_address'].'</td><td>'.$rows['parts_code'].'</td><td>'.$rows['parts_name'].'</td><td>'.$rows['kanban_num'].'</td><td>'.$rows['quantity'].'</td><td>'.$rows['scooter_station'].'</td><td>'.$truck_no.'</td><td>'.date_format($rows['scan_date_time'],"Y-m-d H:i:s").'</td><td>'.date_format($rows['request_date_time'],"Y-m-d H:i:s").'</td><td>'.$print_date_time.' <span class="badge badge-danger" style="cursor:pointer;" onclick="open_reprint_history(&quot;'.$rows['id_scanned_kanban'].'~!~'.$rows['kanban'].'~!~'.date_format($rows['scan_date_time'],"Y-m-d H:i:s").'~!~'.date_format($rows['request_date_time'],"Y-m-d H:i:s").'~!~'.$rows['scooter_station'].'~!~'.$rows['status'].'&quot;)">'.$count_reprint.'</span></td><td style="cursor:pointer;" onclick="add_remarks_mm(&quot;'.$rows['id_scanned_kanban'].'~!~'.$rows['kanban'].'~!~'.$rows['kanban_num'].'~!~'.date_format($rows['scan_date_time'],"Y-m-d H:i:s").'~!~'.date_format($rows['request_date_time'],"Y-m-d H:i:s").'~!~'.$rows['scooter_station'].'&quot;)">'.$mm_remarks.'</td><td>'.$rows['status'].'</td>
 				</tr>
 			';
        }
    }else{
	...
	$limiter_count = $_GET['limiter_count'];
	if($status_select == 'Pending'){
		$sql = "SELECT id FROM mm_scanned_kanban WHERE status = 'Pending' AND scooter_station LIKE '%$scooter_area_select%' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to'";
	 }elseif($status_select == 'Ongoing Picking'){
		$sql = "SELECT id FROM mm_ongoing_picking WHERE status = 'Ongoing Picking' AND scooter_station LIKE '%$scooter_area_select%' AND line_no LIKE '%$line_no%' AND parts_name LIKE '%$parts_name%' AND parts_code LIKE '%$parts_code%' AND scan_date_time >='$date_from' AND scan_date_time <='$date_to'";
	 }
    $params = array();
